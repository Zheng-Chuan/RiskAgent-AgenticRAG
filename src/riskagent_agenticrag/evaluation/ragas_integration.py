...
def try_compute_ragas_metrics(
    *,
    ...
    Supports all major RAGAS metrics including context_precision when reference_contexts provided.
    ...
    Returns:
        ...
        - context_precision: Precision of retrieved contexts vs reference contexts (0-1, requires reference_contexts)
    """
    import warnings
    ...
    for s in samples:
        ...
        # 使用 reference_contexts 作为 reference (for context_precision)
        ref_ctx = s.get("reference_contexts")
        if isinstance(ref_ctx, list) and ref_ctx:
            # context_precision expects a single reference string or list
    ...
    # context_precision 需要 reference_contexts
    if include_reference_based and has_reference_contexts:
        # ContextPrecision measures precision of retrieved contexts vs reference
        precision = context_precision.__class__()
        precision.llm = judge_llm
...
def get_ragas_metrics_description() -> dict[str, str]:
    ...
    return {
        ...
        "ragas_context_recall": "RAGAS Context Recall - Measures recall of relevant contexts vs reference answer (0-1, higher is better, requires reference_answer)",
        "ragas_answer_correctness": "RAGAS Answer Correctness - Measures correctness vs reference answer (0-1, higher is better, requires reference_answer)",
        "ragas_context_precision": "RAGAS Context Precision - Measures precision of retrieved contexts vs reference contexts (0-1, higher is better, requires reference_contexts)",
    }
