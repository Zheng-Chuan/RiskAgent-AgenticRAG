...
def build_retriever(*, persist_dir: Path, final_k: int = 4):
    ...
    base_final_k = max(int(final_k), int(os.getenv("RISKAGENT_PIPELINE_BASE_FINAL_K", str(max(12, int(final_k) * 3)))))
    cfg = HybridConfig(
        dense_k=dense_k,
        sparse_k=sparse_k,
    ...
    hybrid = HybridRetriever(dense_retriever=dense_retriever, sparse_docs=sparse_docs, config=cfg)
    ...
    query_intel = QueryIntelligentRetriever(base_retriever=hybrid, config=qi_cfg)
    ...
    return AdvancedIndexRetriever(base_retriever=query_intel, persist_dir=persist_dir, config=adv_cfg)
