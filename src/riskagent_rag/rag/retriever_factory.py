...
def build_retriever(*, vectorstore: Any, persist_dir: Path, final_k: int = 4) -> Any:
    ...
    if mode not in {"hybrid", "hybrid_rerank", "step1"}:
    ...
    if mode == "hybrid":
        reranker_model = ""
    if mode == "step1" and not reranker_model:
    ...
    return HybridRetriever(dense_retriever=dense_retriever, sparse_docs=sparse_docs, config=cfg)
